...
export default function Contact() {
  ...
  const submit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const {
        error
      } = await supabase.functions.invoke("submit-message", {
  ...
  return <>
      <Seo title="Contact Pet Web Designer in Northampton | Tail Wagging Websites" description="Book a free pet website consultation in Northampton. Request a quote for dog walkers, groomers, sitters & vets across Northamptonshire. View our service area map and contact details." path="/contact" keywords={[
    // Primary intent + locale
    "contact pet web designer northampton", "pet website consultation northampton", "northampton pet web design contact", "pet business website quote northampton", "book pet web design consultation", "request pet website quote", "pet web design service area map", "northampton pet web design location",
    // Niche services
    "dog walker website consultation", "dog groomer website designer northampton", "pet sitter website designer northampton", "veterinary website design northampton", "dog daycare website design", "pet grooming website design", "vet practice website designer", "animal care website design",
    // Local discovery & service-area terms
    "pet web design agency northamptonshire", "northamptonshire pet web design", "local pet website developer", "pet website designer near me northampton",
    // Outcome/benefit terms (intent)
    "seo for pet businesses northampton", "responsive pet website design", "pet business branding and websites", "web design for dog walkers northampton", "pet website contact northampton"]} breadcrumbs={[{
      name: "Home",
      item: "/"
    }, {
      name: "Contact",
    ...
    // Breadcrumbs
    {
      ...
      "itemListElement": [{
        ...
        "position": 1,
        "name": "Home",
        "item": "https://tailwagginwebdesign.com/"
      }, {
        "@type": "ListItem",
        "position": 2,
        "name": "Contact",
        "item": "https://tailwagginwebdesign.com/contact"
      }]
    },
    // FAQ (boosts long-tail; keep answers concise on page)
    {
      ...
      "mainEntity": [{
        ...
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "We start with a free consultation. Packages scale from Starter to Enterprise based on bookings, reviews, and automations."
        }
      }, {
        "@type": "Question",
        ...
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "Yes—our niche is pet-care. We build for dog walkers, groomers, sitters and veterinary practices with local SEO included."
        }
      }, {
        "@type": "Question",
        ...
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "Yes. We’ll run a quick performance and SEO audit and share clear next steps before any build."
        }
      }]
    }]} />
      ...
      <section className="mx-auto max-w-6xl px-4 py-10">
        ...
        <form ref={formRef} onSubmit={submit} className="mt-6 grid gap-4 sm:grid-cols-2" aria-label="Contact form">
          ...
          <div className="flex items-center gap-3 sm:col-span-2">
            <button type="submit" disabled={isSubmitting} className="bg-gradient-primary text-white px-6 py-3 rounded-2xl font-medium shadow-glow hover:scale-105 transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-50 disabled:hover:scale-100">
              {isSubmitting ? 'Sending...' : 'Send Message'}
            </button>
            <CalendlyEmbed buttonText="Book a Free 20-Minute Consult" buttonVariant="secondary" buttonSize="lg" />
          </div>
        </form>
      ...
      <section className="mx-auto max-w-6xl px-4 py-10">
        ...
        <div className="grid gap-6 max-w-4xl">
          <details className="border border-border rounded-lg p-6 group" onToggle={e => trackFAQToggle("How much does a pet website cost in Northampton?", "contact", e.currentTarget.open ? 'open' : 'close')}>
          ...
          <details className="border border-border rounded-lg p-6 group" onToggle={e => trackFAQToggle("Do you work with dog walkers, groomers and vets?", "contact", e.currentTarget.open ? 'open' : 'close')}>
            <summary className="text-lg font-semibold mb-2 cursor-pointer hover:text-primary transition-colors">
          ...
          <details className="border border-border rounded-lg p-6 group" onToggle={e => trackFAQToggle("Can you audit my current site first?", "contact", e.currentTarget.open ? 'open' : 'close')}>
            <summary className="text-lg font-semibold mb-2 cursor-pointer hover:text-primary transition-colors">
